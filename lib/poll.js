...
async function addcandidate(kill, message, candi, pollfile, voterslistfile) {
    ...
    if (data['candis'] === 'null') {
        ...
        if (data['candis'].length >= 10) {
            kill.reply(message.chatId, '🤝O maximo de candidatos que posso adicionar é 10, desculpe.', message.id, true);
            return;
        }
        let cd = {
            name: candi,
    ...
    kill.reply(message.chatId, `🤝Ok! Candidato ${candi} adicionado!`, message.id, true);
}
