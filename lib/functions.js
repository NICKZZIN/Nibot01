...
const wall = async(query) => { // Sobre o comando de Wallpapers
    ...
    const response = await fetch(`https://wall.alphacoders.com/big.php?i=1127177`) // Troque o API por uma API do site alpha coders
...
const ss = async(query) => { // Um print de site
    request({
        ...
        encoding: "binary",
        qs: {
            access_key: "https://api.apiflash.com/v1/urltoimage?access_key=a86bab69eb7e4d5eb09d3b3eb4a6c28b&url=http://google.com", // Insira uma api do site apiflash url to image dentro das aspas
            url: query
        }
    }, (error, response, body) => {
        if (error) {
