...
module.exports = kconfig = async (kill, message) => {
    try {
        ...
        const ownerNumber = '5512997277680@c.us'
        ...
        const mess = {
            ...
            error: {
				...
				Ba: 'Caro administrador, se quiser que eu use esses comandos, precisa me deixar ser uma ademiro!👺',
		...
		case 'about':
			await kill.sendFile(from, './lib/media/img/Nibot.png', 'Nibot.png', sobre, id)
			break
		...
		case 'barzin':
			...
			kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/KYrqSWuemcsA7IDjrH7kBN', 'Que otimo que se interessou pelo Barzin!\nAi está nosso grupo!', id)
			break
       ...
       case 'translate':
        ...
        case 'Nibot':
			try {
				const Nibot = await axios.get(`http://simsumi.herokuapp.com/api?text=${body.slice(6)}&lang=pt`)
				if (Nibot.data.success == '') {
					console.log('Request falhou, usando respostas locais...')
					...
					await kill.reply(from, Nibot.data.success, id)
				}
			} catch (error) {
					console.log('Request falhou, usando respostas locais...')
					let rndrl = fs.readFileSync('./lib/reply.txt').toString().split('\n')
        ...
        case 'criador':
            kill.sendContact(from, '5512997377680@c.us')
		...
		case 'doar':
            ...
            kill.sendContact(from, '5512997277680@c.us')
            break
...
/*        case 'loli':
		...
		case 'chance':
			...
			await kill.reply(from, `_De acordo com meus calculos super avançados de ~macaco macho~ robô "homi" a chance de..._ \n\n*"${body.slice(8)}"*\n\n_...ser realidade é de_ *${lvpc}%.*`, id)
			break
